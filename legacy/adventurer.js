...
function find_sells() {//parent.trade_sell("trade15","MoneyS","v6Rv",$(".tradenum").html())
...
function find_random_pack(type) {//shamelessly stolen from smart_move
...
function simple_move(target) {
    ...
    if (target && (!is_in_range(target) || (target.mtype && target.mtype.includes("fairy")) || (is_best_target(target) && local_obliteration_value != "xscorpion")) && is_good_target(target)) {
        ...
        if (wurst_dest.map == character.map && Math.abs(wurst_dest.x - character.x) <= 25 && Math.abs(wurst_dest.y - character.y) <= 25)
            return;
        if (character.map == target.map && can_move_to(target.real_x, target.real_y)) {// Walk a few steps towards target
        ...
        for (let some_pack of all_packs) {
            ...
            for (var i = 0; i < 4; i++) {
                if (i == 0 && target_type == "goo")
                    i = 1;
                var x_safe, y_safe;
                if (i % 2 == 0) {//0,2 choose left or right
                    x_safe = some_pack[i];
                    y_safe = y_mid
...
//TODO this works worse than it used to
    ...
    if (character.ctype == "rogue") {
        ...
        for (name in parent.entities) {
            ...
            if (!get_party()[name] && !trusted_partners.includes(name) && !(parent.X.characters.some(char => {
    ...
    if (get_party()[local_obliteration_value]) {//this largely works, but problem is how do i know here is when use transport
    ...
    if (!target && !smart.moving && !safety && local_obliteration_value) {//some things do not correctly compute a safety
        smart_comment = -5;
...
function on_party_invite(name) {
    if (parent.X.characters.some(char => {
        return char.name == name;
    }))
...
character.on("hit", x => {
    if (parent.entities[x.actor] && parent.entities[x.actor].player && (!get_party()[x.actor] && !(parent.X.characters.some(char => {
        return char.name == x.actor;
    })))) {
